...
async function getLowPriceCoins() {
  try {
    ...
    const result = lowPriceCoins
      ...
      .filter((c) => c.price >= 100 && c.price < 1000000)
...
async function scanHighCoins() {
  ...
  for (const coin of coins) {
    ...
    if (recommendation === 'STRONG BUY') {
      ...
      sellList.push(coin.symbol)
    } else if (recommendation === 'STRONG SELL') {
      strongSellList.push(coin.symbol)
    }
  }
  ...
  if (strongSellList.length > 0) {
    console.log('\n❌ STRONG SELL:')
    strongSellList.forEach((s) => console.log(`- ${s}`))
  }

  if (sellList.length > 0) {
    console.log('\n🔴 SELL:')
    sellList.forEach((s) => console.log(`- ${s}`))
  }
  ...
  if (strongSellList.length === 0 && sellList.length === 0) {
    console.log('Tidak ada coin dengan sinyal SELL atau STRONG SELL.')
  }
}
